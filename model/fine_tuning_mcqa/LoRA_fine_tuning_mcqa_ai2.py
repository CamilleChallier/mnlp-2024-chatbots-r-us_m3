...
#? train_data_ai2 :  dict_keys(['input_ids', 'attention_mask', 'labels'])
train_data_ai2 = tokenizer(text=prompting_inputs_train, text_target=labels_train, padding=True, truncation=True)
#? val_data_ai2 :  dict_keys(['input_ids', 'attention_mask', 'labels'])
...
training_args = Seq2SeqTrainingArguments(
    output_dir=f"{model_name}-mcqa-ai2-LoRA",
    report_to="tensorboard",
    logging_dir="./tensorboard/"+model_name+"-mcqa-ai2-LoRA",
...
trainer.train()
# save the model
trainer.save_model(f"models/{model_name}-mcqa-ai2-LoRA")
# save the tokenizer
tokenizer.save_pretrained(f"models/{model_name}-mcqa-ai2-LoRA")
