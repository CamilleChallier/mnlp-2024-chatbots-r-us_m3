...
model_checkpoint = "/home/ckalberm/project-m3-2024-chatbots-r-us/models/flan-t5-large_mcqa-ai2-LoRA-merged"
model_name = "flan-t5-large-mcqa-ai2"
...
# ===========================================
# Create the dataset -> sciq
# ===========================================

# load the train and validation datasets from the jsonl file
dataset_train_sciq = helper.read_jsonl_file("/home/ckalberm/project-m3-2024-chatbots-r-us/model/datasets/mcqa/train/mcqa_sciq_train.jsonl")
dataset_evaluation_sciq = helper.read_jsonl_file("/home/ckalberm/project-m3-2024-chatbots-r-us/model/datasets/mcqa/evaluation/mcqa_sciq_validation.jsonl")

# get questions and answers from the datasets
questions_train, answers_train, subjects_train = helper.mcqa_split_dataset(dataset_train_sciq)
questions_val, answers_val, subjects_val = helper.mcqa_split_dataset(dataset_evaluation_sciq)
...
train_data_sciq = tokenizer(text=prompting_inputs_train, text_target=labels_train, padding=True, truncation=True)
#? val_data_sciq :  dict_keys(['input_ids', 'attention_mask', 'labels'])
val_data_sciq = tokenizer(text=prompting_inputs_val, text_target=labels_val, padding=True, truncation=True)

# create the datasets
train_dataset_sciq = Dataset.from_dict({'input_ids': train_data_sciq['input_ids'], 'attention_mask': train_data_sciq['attention_mask'], 'labels': train_data_sciq['labels']})
val_dataset_sciq = Dataset.from_dict({'input_ids': val_data_sciq['input_ids'], 'attention_mask': val_data_sciq['attention_mask'], 'labels': val_data_sciq['labels']})
...
training_args = Seq2SeqTrainingArguments(
    output_dir=f"{model_name}-mcqa-sciq-LoRA",
    report_to="tensorboard",
    logging_dir="./tensorboard/"+model_name+"-mcqa-sciq-LoRA",
...
trainer = Seq2SeqTrainer(
    ...
    model=lora_model,
    args=training_args,
    train_dataset=train_dataset_sciq,
    eval_dataset=val_dataset_sciq,
    tokenizer=tokenizer,
...
trainer.train()
# save the model
trainer.save_model(f"models/{model_name}-mcqa-sciq-LoRA")
# save the tokenizer
tokenizer.save_pretrained(f"models/{model_name}-mcqa-sciq-LoRA")
